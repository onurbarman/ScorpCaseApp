...
class MainActivity : AppCompatActivity() {
            ...
            recyclerViewPeople.run {
                ...
                paginator = object : RecyclerViewPaginator(this) {
                    override val isLastPage: Boolean
                        get() = personAdapter.differ.currentList.size >= dataSource?.getPeopleCount() ?: DataSource.Constants.minPeopleSize
    ...
    private fun setUserList(userList: List<Person>) {
        ...
        if (userList.isNotEmpty()) {
            val mutableList = userList.toList()
            personAdapter.submitList(mutableList)
        }
    }
